...
public class CustomCookieStore implements CookieStore {
    ...
    private final Map<HttpCookie, HttpCookieInfo> cookieInfoJar;
    ...
    public CustomCookieStore() {
        ...
        cookieInfoJar = new LinkedHashMap<>();
    ...
    public Map<HttpCookie, HttpCookieInfo> getCookieInfoJar() {

        lock.lock();

        try {
            return Map.copyOf(cookieInfoJar);
        } finally {
            lock.unlock();
        }
    }

    public Map<URI, List<HttpCookie>> getUriIndex() {

        lock.lock();

        try {
            var copy = new HashMap<URI, List<HttpCookie>>(uriIndex.size());
            uriIndex.forEach((uri, cookies) -> copy.put(uri, List.copyOf(cookies)));
    ...
    public void add(URI uri, HttpCookie cookie, HttpCookieInfo cookieInfo) {
        ...
        try {
            ...
            cookieInfoJar.remove(cookie);

            // add new cookie if it has a non-zero max-age
            if (cookie.getMaxAge() != 0) {
                cookieJar.add(cookie);
                cookieInfoJar.put(cookie, cookieInfo);
    ...
    public void add(URI uri, HttpCookie cookie) {
        ...
        try {
            ...
            if (cookie.getMaxAge() != 0) {
                cookieJar.add(cookie);
                cookieInfoJar.put(cookie, new HttpCookieInfo(System.currentTimeMillis(), cookie.toString()));
                // and add it to domain index
                if (cookie.getDomain() != null) {
                    addIndex(domainIndex, cookie.getDomain(), cookie);
                }
